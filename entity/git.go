...
func update(repo Repo, cloneDir string) error {
	...
	internal.Logger.Info().Str("name", repo.Name).Msg("Updating repo")
...
func clone(repo Repo, repoUrl cloneRef, cloneDir string) error {
	...
	internal.Logger.Trace().Str("name", repo.Name).Str("path", cloneDir).Msg("Cloning repo")
	...
	for remote, remoteUrl := range repo.Remotes {
		...
		internal.Logger.Trace().Str("remote", remote).Str("url", remoteUrl).Msg("Fetching remote")
